...
line_bot_api = LineBotApi('kE1aKDcsEa1B9kpaAGvH7PGN3CwyKVQhCOZWRiKRkbHFHou/Rj8Lf6tjv32sXIS5B4LpM81L3aJbcqYjiVTp9OH1xo59Xx2swMobhzFCDg5C6SC07TC2DWNpqABkMPN7pfmZD+iQEiC0jNiO8IIBggdB04t89/1O/w1cDnyilFU=')
handler = WebhookHandler('02b1c15c90c43b8e64ef0570a1e9c3dc')
...
def handle_message(event):
    ...
    if event.message.text == "我在哪":
        line_bot_api.reply_message(event.reply_token,TextSendMessage(text='開山里'))
    elif event.message.text == "...":
        line_bot_api.reply_message(event.reply_token,TextSendMessage(text='用好了沒'))
    elif event.message.text == "貼圖":
        line_bot_api.reply_message(event.reply_token,StickerSendMessage(package_id=1, sticker_id=2))
#    elif event.message.text == "圖片":
#        line_bot_api.reply_message(event.reply_token,ImageSendMessage(original_content_url='https://www.google.com/url?sa=i&source=images&cd=&ved=2ahUKEwii-fjYtKzjAhUW9bwKHVshBAUQjRx6BAgBEAU&url=http%3A%2F%2F90sheji.com%2Fpng%2Ficontianqi.html&psig=AOvVaw3eFEKkVWn0f1DhpOqMyy2x&ust=1562918822834221', preview_image_url='https://www.google.com/url?sa=i&source=images&cd=&ved=2ahUKEwii-fjYtKzjAhUW9bwKHVshBAUQjRx6BAgBEAU&url=http%3A%2F%2F90sheji.com%2Fpng%2Ficontianqi.html&psig=AOvVaw3eFEKkVWn0f1DhpOqMyy2x&ust=1562918822834221'))
    ...
    elif event.message.text == "怎麼都是開山里":
        line_bot_api.reply_message(event.reply_token,ImageSendMessage(original_content_url='https://imgs.niusnews.com/upload/imgs/default/2019JULY_Arena/OPEN/6.png', preview_image_url='https://imgs.niusnews.com/upload/imgs/default/2019JULY_Arena/OPEN/6.png'))
    return 'OK2'
'''
'''
統一回復
...
def check_intent(intent,parameters):
    ...
    if intent == "take_information" or intent == "return":#取還資訊
        if len(parameters.get("airport")) != 0: 
    ...
    for i in range(len(fulfillmentText)):
        fulfillmentMessages.append({"text": {"text": [fulfillmentText[i]]},"platform": "LINE"})    
    return fulfillmentMessages
def makeWebhookResult(req):
    ...
    #假設我去抓資料寫這
    fulfillmentMessages = check_intent(intent,parameters)
    original_fulfillMessages = result.get("fulfillmentMessages")
    for i in range(len(original_fulfillMessages)):
        fulfillmentMessages.append(original_fulfillMessages[i])

    print("Response:")
    print(fulfillmentMessages)
    #回傳
    return {"fulfillmentMessages": fulfillmentMessages}#'fulfillmentText': fulfillmentText
